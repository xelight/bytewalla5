				...
				if (DTNService.is_running()) {
					...
					// show progress dialog
						LoadingDialog = ProgressDialog.show(self_, "Processing", "Starting DTNService...", true,
			    		 ...
			    		 new Thread(new Runnable() {
			    			public void run() {
			    				Message message = Message.obtain();  //for message control and managemet
	...
	private void start_DTN_service() throws FileNotFoundException,
		...
		try {
			in = this.getAssets().open(
					getResources().getString(R.string.DTNConfigFilePath)); //dtn.config.xml
		} catch (IOException e) {
		...
		startService(DTNServer_intent_); //DTNServer_intent_ = new Intent(DTNManager.this, DTNService.class);
