...
public class UDPConnection extends CLConnection {
	 ...
	 */
	private final static String TAG = "UDPConnection";
	 ...
	 * Constructor.
	 */
	public UDPConnection(StreamConvergenceLayer cl, StreamLinkParams params,
	 ...
	 * Set the socket
	 */
	public void set_socket(DatagramSocket socket) {
		socket_ = socket;
	}
	...
	void send_data() {
		...
		try {
			...
			byte b[] = new byte[temp.position()];
			for (int i = 0; i < b.length; i++)
				b[i] = temp.array()[i];

			DatagramPacket pack = new DatagramPacket(b, b.length,
					params_.remote_addr_, params_.remote_port_);
			socket_.send(pack);
