...
import se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.security.Ciphersuite;
import se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.security.KeyDB;
...
public class BundleDaemon extends BundleEventHandler implements Runnable {
	...
	public void do_init() {
		...
		try// added by ak
		{
		Ciphersuite.init_default_ciphersuites();//initial block cipher
		SPD.getInstance().init();//just get an spd instance
	    KeyDB.getInstance().init();//just get an instance
		}
		catch(Exception e)
		{
			Log.i(TAG, String.format( "error"));//added by AK for debug
		}
