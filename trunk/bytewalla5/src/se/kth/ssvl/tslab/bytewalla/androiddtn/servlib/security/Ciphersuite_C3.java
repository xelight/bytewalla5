...
import static se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.bundling.BlockProcessor.BP_FAIL;
import static se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.bundling.BlockProcessor.BP_SUCCESS;
...
public class Ciphersuite_C3 extends Ciphersuite 
{
	...
	{
		Log.d(TAG, String.format( "Ciphersuite_C3::consume()"));
		int cc = block.owner().consume(bundle, block, buf, len);

		if (cc == -1) {
			Log.i(TAG, String.format( "breakpoint 1, protocol error"));//added by AK for debug
		...
		if (! block.complete()) 
		{
			Log.i(TAG, String.format( "breakpoint 2, traverse the blockon the fly(incoming)"));//added by AK for debug
			assert(cc == (int)len);
		...
		{   
			Log.i(TAG, String.format( "breakpoint 23 traverse the block"));//added by AK for debug
			parse(block);
		}
	...
	{
		Log.i(TAG, String.format( "breakpoint 4,  error"));//added by AK for debug
		int sdnv_len = SDNV.decode(buf, val);
		if(sdnv_len<0 ){
			Log.e(TAG, String.format( "Block processor too short exception"));
		}
		assert(sdnv_len < 0):
			...
			status_report_reason_t[] deletion_reason)
	{
		...
		Log.i(TAG, String.format( "breakpoint 5,  error"));//added by AK for debug
		...
		Log.d(TAG, String.format( "validate() "));
		...
		{  //yes - this is ours so go to work
			...
			Log.i(TAG, String.format( "breakpoint 6,  error"));//added by AK for debug
			...
			Log.d(TAG, String.format( "validate() locals.correlator() 0x%x", (locals.correlator())));
			Log.d(TAG, String.format( "validate() security params, len = %d", len));
			while ( len > 0 ) 
			{
				...
				case CS_IV_field: 
				{
					Log.d(TAG, String.format( "validate() iv item, len = %d", (field_length)));
				...
				{
					Log.d(TAG, String.format( "validate() salt item, len = %d", (field_length)));
				...
				{
					Log.d(TAG, String.format( "validate() frag info item, len = %d", (field_length)));
				...
				default:    // deal with improper items
					Log.e(TAG, String.format( "validate: unexpected item type %d in security_params",
							item_type));
			...
			Log.d(TAG, String.format( "validate(). security result, len = %d", len));
			while ( len > 0 ) 
			{
				Log.d(TAG, String.format( "remaining len = %d", len));
				...
				field_length=value[0];
				Log.d(TAG, String.format( "field length: "+field_length));
				len -= sdnv_len;
				...
				{
					Log.d(TAG, String.format( "validate() key ID item"));
					Log.e(TAG, String.format( "sd. not in specs!!! Error."));
				...
				{
					Log.d(TAG, String.format( "validate(). Parsing encoded key item"));
					db= new SerializableByteBuffer(16);

					try {
						KeySteward.decrypt(bundle, locals.security_src(), buf, field_length, db);
					} catch (Exception e) {
						Log.i(TAG, String.format( "null 2"));// added by ak for debug
						Log.e(TAG,"The key could not be decrypted. Deleting bundle...");
				...
				{
					Log.d(TAG, String.format( "validate(). Parsing icv item"));
				...
				case CS_encap_block_field:
				{
					...
					Log.e(TAG, String.format( "validate(). unexpected encap block in security_result"));
					if ( locals !=  null ) //GOTO FAIL
						locals.set_proc_flags ((short) (proc_flags_t.CS_BLOCK_FAILED_VALIDATION.getCode() | proc_flags_t.CS_BLOCK_COMPLETED_DO_NOT_FORWARD.getCode()));
					return false;
				}
				...
				default:    // deal with improper items
					Log.e(TAG, String.format( "validate: unexpected item type %d in security_result",
							item_type));
					if ( locals !=  null ) //GOTO FAIL
						locals.set_proc_flags ((short) (proc_flags_t.CS_BLOCK_FAILED_VALIDATION.getCode() | proc_flags_t.CS_BLOCK_COMPLETED_DO_NOT_FORWARD.getCode()));
					return false;
			...
			Log.d(TAG, String.format( "validate(). walk block list"));
			...
			while  (blocks_iter.hasNext())
			{
				...
				{
					Log.d(TAG, String.format( "validate() C block "));
					Log.e(TAG, String.format( "Nested Confidentiality blocks support has not been tested"));
				...
				{
					System.gc();
					Log.d(TAG, String.format( "validate(). PAYLOAD_BLOCK"));
					...
					Log.e(TAG, String.format(  "Decrypted symmetric key: 0x %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h", unsignedByteToInt(temp_key_array[0]), unsignedByteToInt(temp_key_array[1]), unsignedByteToInt(temp_key_array[2]), unsignedByteToInt(temp_key_array[3]), unsignedByteToInt(temp_key_array[4]), unsignedByteToInt(temp_key_array[5]), unsignedByteToInt(temp_key_array[6]), unsignedByteToInt(temp_key_array[7]), unsignedByteToInt(temp_key_array[8]), unsignedByteToInt(temp_key_array[9]), unsignedByteToInt(temp_key_array[10]), unsignedByteToInt(temp_key_array[11]), unsignedByteToInt(temp_key_array[12]), unsignedByteToInt(temp_key_array[13]), unsignedByteToInt(temp_key_array[14]), unsignedByteToInt(temp_key_array[15])));
					...
					mutate_func do_encrypt = new mutate_func() 
					{
						...
						{
							Log.i(TAG, String.format( "breakpoint 7, protocol error"));//added by AK for debug
							//initialization
							...
							Log.e(TAG, String.format(  "do_crypt(): Encrypted payload (first 10 bytes max): 0x "+encr_payl));
							...
							Log.d(TAG, String.format(  "do_crypt(): operation decryption, len %d",  len));
					...
					Log.e(TAG, String.format(  "validate(): Decrypted Plaintext (first 10 max)[hex]: 0x "+payload_hex));
					Log.e(TAG, String.format(  "validate(): Decrypted Plaintext message (first 10 max)[string]: "+payload_str));
			...
			Log.d(TAG, String.format( "validate() walk block list done"));
		...
		Log.d(TAG, String.format( "validate()  done"));
		return true;
	}
	...
	{
		Log.d(TAG, String.format(  "prepare()"));
		int             result = BP_FAIL;
		short       cs_flags = 0;
		...
		{
			Log.d(TAG, String.format(  "prepare() - not being forwarded"));
		...
		if ( list == BlockInfo.list_owner_t.LIST_RECEIVED ) //executed only when we are bypassing a security block. Not tested because I'd need 3 android phones.
		{
			...
			Log.d(TAG, String.format(  "prepare() - forward received block len %d eid_list_count %d new count %d",
				...
				else 
				{
					...
					else 
					{
						...
						if ( (source_locals.cs_flags() & ciphersuite_flags_t.CS_BLOCK_HAS_SOURCE.getCode())>0 ) 
						{
							...
							{
								Log.d(TAG, String.format(  "prepare() add security_src EID"));
							...
							if ( (source_locals.cs_flags() & ciphersuite_flags_t.CS_BLOCK_HAS_DEST.getCode())>0 ) 
							{
								...
								{
									Log.d(TAG, String.format(  "prepare() add security_dest EID"));
								...
								Log.d(TAG, String.format(  "prepare() - inserted block eid_list_count %d", bp.eid_list().size()));
								result = BP_SUCCESS;
								return result;
							}
						}
		...
		else //BlockInfo.list_owner_t is not LIST_RECEIVED, i.e. it is a newly created block.
		{
			...
			Log.d(TAG, String.format(  "prepare() - add new block (or API block etc)"));
			...
			else 
			{
				...
				else 
				{
					...
					Log.d(TAG, String.format(  "prepare() local_eid %s bundle.source_ %s", local_eid.toString(), bundle.source().toString()));
					...
					{
						Log.d(TAG, String.format( "prepare() add security_src EID %s", locals.security_src().toString()));
						cs_flags |= ciphersuite_flags_t.CS_BLOCK_HAS_SOURCE.getCode();
					...
					{
						Log.d(TAG, String.format(  "prepare() add security_dest EID %s", locals.security_dest().toString()));
						cs_flags |= ciphersuite_flags_t.CS_BLOCK_HAS_DEST.getCode();
			...
			boolean           last) 
	{
		...
		Log.d(TAG, String.format(  "generate() "));
		...
		if ( locals.list_owner() == BlockInfo.list_owner_t.LIST_RECEIVED ) //executed only when we are bypassing a security block. Not tested because I'd need 3 android phones.
		{
			...
			Log.d(TAG, String.format(  "generate()  done"));
			return BP_SUCCESS;
		}
		...
		if ( need_correlator ) 
		{
			...
			Log.d(TAG, String.format(  "generate() correlator "+ correlator));
		}
		...
		{
			Log.e(TAG, String.format( "Error. fragments not implemented."));
			/*
		...
		Log.d(TAG, String.format(  "generate() security params, len = %d", param_len));
		...
		Log.d(TAG, String.format(  "generate() random key: 0x %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h", unsignedByteToInt(key[0]), unsignedByteToInt(key[1]), unsignedByteToInt(key[2]), unsignedByteToInt(key[3]), unsignedByteToInt(key[4]), unsignedByteToInt(key[5]), unsignedByteToInt(key[6]), unsignedByteToInt(key[7]), unsignedByteToInt(key[8]), unsignedByteToInt(key[9]), unsignedByteToInt(key[10]), unsignedByteToInt(key[11]), unsignedByteToInt(key[12]), unsignedByteToInt(key[13]), unsignedByteToInt(key[14]), unsignedByteToInt(key[15])));
		...
		Log.d(TAG, String.format("generate():  sec_dest: "+sec_dest));
		try
		{
			err = KeySteward.encrypt(bundle, null, link, sec_dest, key, key.length, encrypted_key);
		}
		catch (Exception e) 
		{
			Log.e(TAG,e.toString());
			Log.i(TAG, String.format( "NUll pointer caught here"));//added by ak for debug
		...
		Log.d(TAG, String.format(  "generate(): encrypted_key len: %d", encrypted_key.capacity()));

		res_len = 1 + SDNV.encoding_len(encrypted_key.capacity()) + encrypted_key.capacity(); //we calculate the length of the security-result field: we consider encrypted key and ICV. 
		res_len += 1 + 1 + tag_len;
		...
		length = 0;       
		if ( need_correlator ) {
			Log.e(TAG, String.format(  "generate() algo anda mal, porq usa correlator.", (correlator)));
			Log.d(TAG, String.format(  "generate() correlator %llu", (correlator)));
			locals.set_correlator(correlator);
		...
		Log.d(TAG, String.format(  "generate() preamble has len: %d. block has len: %d", block.data_offset(), length));
			...
			final Link link) 
	{
		...
		Log.d(TAG, String.format( "finalize()"));
		Log.e(TAG, String.format( "Starting encryption process..."));
		...
		Log.d(TAG, String.format( "finalize()  we start walking the list of blocks"));
		...
		while(blocks_iter.hasNext())
		{
			...
			Log.d(TAG, String.format( "finalize()  itaration.next. type of block: "+iter.type().toString()));
				...
				//if ( (iter) <= block ) 
			{
				...
				{
					Log.d(TAG, String.format( "finalize()  we got a security block.skiping"));
				...
				{
					Log.d(TAG, String.format( "finalize()  we got a confidentialiity block.skiping"));
					target_locals = (BP_Local_CS)(iter.locals());
				...
				if (iter==block)
				{
					Log.d(TAG, String.format( "finalize() this block was our payload security block! Next should be payload"));
					process_blocks = true;
				}
			...
			{
				Log.e(TAG, String.format( "Encapsulated blocks support has not been tested."));
				Log.d(TAG, String.format( "finalize() PSB or CB after our block. we are going to encapsulate it"));
			...
			case PAYLOAD_BLOCK:
			{
				Log.d(TAG, String.format( "finalize(). Payload block after our block. we are going to encrypt it"));
				...
				Log.d(TAG, String.format( "finalize() PAYLOAD_BLOCK"));
				...
				Log.d(TAG, String.format( "finalize(). nonce: 0x %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h ", unsignedByteToInt(nonce.get(0)), unsignedByteToInt(nonce.get(1)), unsignedByteToInt(nonce.get(2)), unsignedByteToInt(nonce.get(3)), unsignedByteToInt(nonce.get(4)), unsignedByteToInt(nonce.get(5)), unsignedByteToInt(nonce.get(6)), unsignedByteToInt(nonce.get(7)), unsignedByteToInt(nonce.get(8)), unsignedByteToInt(nonce.get(9)), unsignedByteToInt(nonce.get(10)), unsignedByteToInt(nonce.get(11))));
				//*****************************************************descomentar aca******************************************            
				//gcm_init_message(nonce, nonce_len, (ctx_ex.c));
				...
				Log.e(TAG, String.format(  "generate() random key: 0x %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h", unsignedByteToInt(temp_key_array[0]), unsignedByteToInt(temp_key_array[1]), unsignedByteToInt(temp_key_array[2]), unsignedByteToInt(temp_key_array[3]), unsignedByteToInt(temp_key_array[4]), unsignedByteToInt(temp_key_array[5]), unsignedByteToInt(temp_key_array[6]), unsignedByteToInt(temp_key_array[7]), unsignedByteToInt(temp_key_array[8]), unsignedByteToInt(temp_key_array[9]), unsignedByteToInt(temp_key_array[10]), unsignedByteToInt(temp_key_array[11]), unsignedByteToInt(temp_key_array[12]), unsignedByteToInt(temp_key_array[13]), unsignedByteToInt(temp_key_array[14]), unsignedByteToInt(temp_key_array[15])));
				//for (int i=0; i<temp_key_array.length;i++)
				//	Log.e(TAG, ""+( unsignedByteToInt(temp_key_array[i])) );
				//Log.d(TAG, "");
				...
				mutate_func do_crypt = new mutate_func() 
				{
					...
					public boolean action(ServlibEventData data) 
					{
						...
						Log.e(TAG, String.format(  "generate(). Plaintext message (first 10 char max): 0x "+key_str));
						...
						Log.d(TAG, String.format(  "Ciphersuite_C3::do_crypt() operation encryption len %d",  len));
				...
				{
					Log.e(TAG,e.toString());
					Log.i(TAG, String.format( "NUll here"));// added by ak for debug
				} 
				...
				Log.e(TAG, String.format(  "generate(): Encrypted payload using symmetric key (10 char max): 0x "+encr_payl));
				...
				Log.d(TAG, String.format( "finalize(): tag: 0x %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h %2.2h ", unsignedByteToInt(tag.get(0)), unsignedByteToInt(tag.get(1)), unsignedByteToInt(tag.get(2)), unsignedByteToInt(tag.get(3)), unsignedByteToInt(tag.get(4)), unsignedByteToInt(tag.get(5)), unsignedByteToInt(tag.get(6)), unsignedByteToInt(tag.get(7)), unsignedByteToInt(tag.get(8)), unsignedByteToInt(tag.get(9)), unsignedByteToInt(tag.get(10)), unsignedByteToInt(tag.get(11)), unsignedByteToInt(tag.get(12)), unsignedByteToInt(tag.get(13)), unsignedByteToInt(tag.get(14)), unsignedByteToInt(tag.get(15))));
				...
				if (!(type.equals(Ciphersuite.ciphersuite_fields_t.CS_encoded_key_field)))//FAIL_IF
				{
					if ( locals !=  null )
						locals.set_proc_flag (proc_flags_t.CS_BLOCK_PROCESSING_FAILED_DO_NOT_SEND.getCode());
					Log.e(TAG, String.format( "finalize(). We should have found CS_encoded_key_field!!")); 
					return BP_FAIL;
				}
				...
				if (rem != 1 + 1 + tag_len)//FAIL_IF. remaining code should be type-length-value of ICV.
				{
					if ( locals !=  null )
						locals.set_proc_flag (proc_flags_t.CS_BLOCK_PROCESSING_FAILED_DO_NOT_SEND.getCode());
					Log.e(TAG, String.format( "finalize(). error in if (rem != 1 + 1 + tag_len)"));
					return BP_FAIL;
				}
				...
				Log.d(TAG, String.format( "finalize(). PAYLOAD_BLOCK done"));
		...
		Log.d(TAG, String.format( "finalize() done"));
