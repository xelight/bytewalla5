...
public class BundleProtocol {
	...
	public static int generate_blocks(Bundle bundle, BlockInfoVec blocks,
	    ...
	    for (int i=0; i < blocks.size(); i++)
	    {
	        ...
	        err = iter.owner().generate(bundle, blocks, iter, link, last);
	        
	        if (err!=0)
	        	Log.e(TAG, String.format("generate_blocks(): ERROR WHILE EXECUTING GENERATE*****************************"));
	        else
	    ...
	    {
	    	BlockInfo iter = blocks.get(i);
	        err=iter.owner().finalize(bundle, blocks, iter, link);
	    	if (err!=0)
	    		Log.e(TAG, String.format("generate_blocks(): ERROR WHILE EXECUTING FINALIZE *****************************"));
	...
	public static int produce(final Bundle bundle, final BlockInfoVec blocks,
		    ...
		    Log.e(TAG, String.format("BundleProtocol::produce complete: " +
		                "produced %d bytes, bundle id %d, status is  %s",
		                origlen - len,
		                bundle.bundleid(), 
		                last[0] ? "complete" : "not complete"));
		    Log.e(TAG, String.format("Sending bundle... [OK]"));
	...
	public static int consume(Bundle bundle, IByteBuffer data, int len,
		    ...
		    while (len != 0) {
		        ...
		        //sd. in C++, "value" is not midified. But in Java, value.pos was modified. We set it back.
		    ...
		    Log.e(TAG, String.format("Receiving bundle... [OK]"));
		    Log.e(TAG, String.format("bundle id %d consume completed, %d/%d bytes consumed %s",
		                bundle.bundleid(),
		    		    origlen - len, 
		                origlen, 
	...
	public static boolean validate(Bundle bundle,
	    ...
	    if (! SPD.getInstance().verify_in_policy(bundle)) 
	    {
	    	Log.e(TAG, String.format("bundle failed security policy verification"));
	    	Toast.makeText(DTNManager.me, "Received bundle failed security policy verification. Deleting...", Toast.LENGTH_SHORT).show();  
	...
	public static enum bundle_block_type_t {
		...
		BUNDLE_AUTHENTICATION_BLOCK ((byte) 2),
	    PAYLOAD_SECURITY_BLOCK ((byte) 3),
	    CONFIDENTIALITY_BLOCK  ((byte) 4),
